...
Result_mean_std <- Result[,grep('mean.|std.',names(Result))]
write.csv(Result_mean_std, 'Tidy Dataset.csv')
# Calculating avg vor each variable for each activity & person
ResultTable <- aggregate(. ~ Person_number+Activity_Name, Result, mean, na.rm = TRUE, na.action="na.pass")
write.csv(ResultTable, 'Tidy Grouped Dataset.csv')